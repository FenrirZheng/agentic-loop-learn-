...
4. **Emit** the finished artifact in a fenced block, with an explicit stop condition and (if any refine loop) a grounded oracle or a stated round cap. Pick the output mode per SKILL.md step 4: a copy-pastable markdown prompt/spec (default), or — when the target is Claude Code multi-agent orchestration — a runnable Workflow `.mjs` script per `skills/agentic-prompt-composer/references/workflow-mjs.md`. In both modes, write the prompt text (including every `agent()` string) to the standard in `skills/agentic-prompt-composer/references/prompt-quality.md` and run its smell test.
